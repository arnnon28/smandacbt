    ...
    window.switchView = function (viewId) {
      ...
      if (viewId === 'student-exam') {
        ...
        if (systemHeader) systemHeader.classList.add('hidden');
        if (systemSidebar) {
          systemSidebar.style.setProperty('display', 'none', 'important');
        }
        if (mainSystemView) {
          mainSystemView.classList.add('unbk-body-bg');
          mainSystemView.classList.remove('bg-slate-100', 'dark:bg-darkBg');
        }
        document.body.classList.add('unbk-body-bg');
        if (typeof lucide !== 'undefined') lucide.createIcons();
      } else {
        document.body.classList.remove('exam-mode');
        if (systemHeader) systemHeader.classList.remove('hidden');
        if (systemSidebar) {
          systemSidebar.style.display = '';
        }
        if (mainSystemView) {
          mainSystemView.classList.remove('unbk-body-bg');
          mainSystemView.classList.add('bg-slate-100', 'dark:bg-darkBg');
        }
        document.body.classList.remove('unbk-body-bg');
      }
    ...
    // ================= EXAM UI FUNCTIONS (exam.html design) =================
    function populateExamHeader() {
      ...
      const settingsLogo = document.getElementById('header-school-logo');
      const settingsName = document.getElementById('header-school-name');
      const settingsTitle = document.getElementById('header-exam-title');

      if (logoEl && settingsLogo) logoEl.src = settingsLogo.src;
      if (nameEl && settingsName) nameEl.textContent = settingsName.textContent;
      if (titleEl && settingsTitle) titleEl.textContent = settingsTitle.textContent;
      if (CURRENT_USER) {
        if (studentNameEl) studentNameEl.textContent = CURRENT_USER.nama || 'Siswa';
        if (studentNameMobileEl) studentNameMobileEl.textContent = CURRENT_USER.nama || 'Siswa';
    ...
    window.setExamFontSize = function (size) {
      const map = { kecil: 'small', sedang: 'medium', besar: 'large' };
      changeExamFontSize(map[size] || 'medium');
    };

    function isExamReadyToFinish() {
      const total = EXAM_STATE.scrambledQuestions.length;
      if (total === 0) return false;
      const allAnswered = EXAM_STATE.scrambledQuestions.every(q => EXAM_STATE.answers[q.id] !== undefined);
      const hasDoubts = EXAM_STATE.scrambledQuestions.some(q => EXAM_STATE.doubts[q.id]);
      return allAnswered && !hasDoubts;
    }
    ...
    function navigateExamQuestion(dir) {
      ...
      if (target >= total) {
        for (let i = 0; i < total; i++) {
          const q = EXAM_STATE.scrambledQuestions[i];
          if (!EXAM_STATE.answers[q.id] || EXAM_STATE.doubts[q.id]) {
    ...
    window.toggleUnbkGrid = function () {
    ...
    function setupSessionEnvironment(user) {
      ...
      if (user.role === 'student') {
        document.body.classList.add('exam-mode');
        if (systemHeader) systemHeader.style.setProperty('display', 'none', 'important');
        if (systemSidebar) systemSidebar.style.setProperty('display', 'none', 'important');
        if (mainSystemView) {
        ...
        if (systemHeader) systemHeader.classList.remove('hidden');
        if (systemSidebar) systemSidebar.style.display = '';
      }
      ...
      if (pContainer) {
        ...
        if (pName && pRole) {
          pName.textContent = user.nama;
          pRole.textContent = 'siswa';
        }
      }

      const studentNavBtn = document.getElementById('btn-mobile-nav');
      document.getElementById('admin-nav-links').classList.add('hidden'); 
      document.getElementById('student-nav-links').classList.remove('hidden'); 
      document.getElementById('admin-active-session-bar').classList.add('hidden');
    ...
    function isStudentExamActive() {
      return CURRENT_USER && CURRENT_USER.role !== 'admin' && sessionStorage.getItem('cbt-exam-active') === '1';
    }

    function markExamPageActive() {
      sessionStorage.setItem('cbt-exam-active', '1');
    }

    function clearExamPageActive() {
      sessionStorage.removeItem('cbt-exam-active');
    }
    ...
    function handleFullscreenChange() {
      if (!document.fullscreenElement && CURRENT_USER && CURRENT_USER.role !== 'admin') {
        ...
        if (EXAM_STATE.cheatTabCount >= 3) {
          showNotification("Curang Terdeteksi", "Anda keluar dari mode Layar Penuh sebanyak 3 kali. Ujian otomatis dikirim.", "danger");
          autoSubmitExam("Keluar Mode Layar Penuh");
        } else {
          showNotification("Peringatan Layar Penuh", `Wajib Fullscreen! Keluar dari mode layar penuh dihitung sebagai pelanggaran. (${EXAM_STATE.cheatTabCount}/3)`, "danger");
    ...
    function collapseUnusedLineBreaks(html) {
      let result = html;
      ...
      result = result.replace(/(?:<br\s*\/?>\s*)+(?=<(?:p|div|h[1-6]|ul|ol|li|table|blockquote)\b)/gi, '');
      result = result.replace(/(<\/(?:p|div|h[1-6]|ul|ol|li|table|blockquote)>)\s*(?:<br\s*\/?>\s*)+/gi, '$1');
      return result.trim();
    }
    ...
    function sanitizeHtmlContent(html) {
      ...
      return collapseUnusedLineBreaks(doc.body.innerHTML);
    }
    ...
    async function handleSaveManualQuestion() {
      ...
      for (const letter of opsiKeys) {
        ...
        if (!optionTextPlain && !/<img[^>]*>/i.test(textRaw)) return showNotification('Isi Jawaban', `Jawaban ${letter} wajib diisi.`, 'danger');
        const text = textRaw ? sanitizeHtmlContent(stripQuestionKeyLabel(textRaw)) : '';
        opsi.push({ key: letter, text, image: null });
      }
    ...
    window.triggerEditSchedule = function (id) {
      ...
      showConfirmation("Edit Jadwal", `
        <div class="space-y-2 text-xs text-left text-slate-800 dark:text-slate-100">
          <label class="font-bold text-slate-400 uppercase tracking-wider block">Mata Pelajaran</label>
          ...
          <label class="font-bold text-slate-400 uppercase tracking-wider block">Paket Soal</label>
          ...
          <label class="font-bold text-slate-400 uppercase tracking-wider block">Mulai</label>
          ...
          <label class="font-bold text-slate-400 uppercase tracking-wider block">Durasi (Menit)</label>
          ...
          <label class="font-bold text-slate-400 uppercase tracking-wider block">Kelas Terpilih</label>
      ...
      const r = new FileReader(); r.onload = function (ev) {
        ...
        mammoth.convertToHtml({ arrayBuffer: ev.target.result }, mammothOptions).then(async (res) => {
          ...
          for (let p of div.querySelectorAll('p')) {
            ...
            let rawHtml = p.innerHTML.trim();
            let txt = rawHtml.replace(/<[^>]*>/g, '').trim(); // Plain text without HTML

            const keyMatch = txt.match(/^(.*?)(?:\s*(?:kunci|jawaban|answer)\s*[:=]?\s*([A-Ea-e])\s*)$/i);
            if (keyMatch && cq) {
              txt = keyMatch[1].trim();
              cq.correct_key = keyMatch[2].toUpperCase();
              rawHtml = rawHtml.replace(/(?:kunci|jawaban|answer)\s*[:=]?\s*[A-Ea-e]/i, '').trim();
            }
            ...
            if (qm) {
              ...
              let optHtml = rawHtml.replace(new RegExp(`^\\s*${om[1]}[\\.\\)\\s]+`, 'i'), '').trim();
              const optionHtml = sanitizeHtmlContent(optHtml || om[2].trim());
              cq.opsi.push({ key: om[1].toUpperCase(), text: optionHtml, image: null });
            } else if (cq) {
              const addedHtml = sanitizeHtmlContent(rawHtml || txt);
              cq.soal += "<br>" + addedHtml;
            }
          }
          if (cq) qs.push(cq);
    ...
    async function handleAddSchedule() {
      ...
      showConfirmation("Jadwal", `<div class="space-y-2 text-xs"><input id="s-m" placeholder="Mapel" class="w-full p-2 border rounded"><select id="s-p" class="w-full p-2 border rounded"><option value="">Pilih</option>${ALL_PACKETS.map(p => `<option value="${p.id_paket}">${p.nama_paket}</option>`).join('')}</select><input type="datetime-local" id="s-dt" class="w-full p-2 border rounded"><input type="number" id="s-d" placeholder="Durasi(Menit)" class="w-full p-2 border rounded"><div class="flex flex-wrap gap-2">${uc}</div><label class="flex items-center gap-1.5 mt-2"><input type="checkbox" id="s-tn" class="rounded text-primary"> <span class="text-[10px] font-bold text-slate-700">Tampilkan Nilai ke Siswa</span></label></div>`, async () => {
        const m = document.getElementById('s-m').value, p = document.getElementById('s-p').value, dt = document.getElementById('s-dt').value, dr = parseInt(document.getElementById('s-d').value, 10), cl = Array.from(document.querySelectorAll('input[name="sc"]:checked')).map(cb => cb.value), tn = document.getElementById('s-tn').checked;
    ...
    function setupInteractiveListeners() {
      ...
      safeAdd('chk-ragu', 'change', (e) => {
        ...
        saveExamStateToLocal();
        renderExamQuestion();
        syncStudentActiveProgress();
      });
      safeAdd('btn-prev-question', 'click', () => navigateExamQuestion(-1));
      safeAdd('btn-next-question', 'click', () => navigateExamQuestion(1));
    ...
    function showExamRulesDialog() {
      showConfirmation("Peringatan..!!", `<div class="dialog-warning-content text-left space-y-3 leading-relaxed">
          <p class="font-extrabold text-rose-600 dark:text-rose-400 uppercase tracking-wide">Larangan &amp; Aturan Keamanan Ujian</p>
          <ul class="list-disc pl-5 space-y-2 font-semibold">
            <li>Wajib dalam mode <strong>Layar Penuh (Fullscreen)</strong> selama ujian berlangsung.</li>
            <li>Dilarang keluar halaman, membuka tab/aplikasi lain, atau meminimalkan browser.</li>
            <li>Dilarang <strong>menutup/mematikan layar HP/PC</strong> secara manual/otomatis (dianggap buka Tab).</li>
            <li>Dilarang menyalin, men-screenshot, memfoto, atau membagikan soal ujian.</li>
            <li>Pelanggaran akan tercatat dan ujian dapat diakhiri otomatis oleh sistem.</li>
            <li>Pastikan koneksi internet stabil sebelum memulai.</li>
          </ul>
          <p class="pt-1 font-bold">Dengan menekan <strong>Lanjutkan</strong>, Anda menyetujui seluruh aturan di atas. Layar akan otomatis masuk mode Layar Penuh.</p>
        </div>`, () => {
        requestFullscreenMode();
    ...
    async function cancelExamSession() {
      toggleLoader(true, "Membatalkan sesi ujian...");
      try {
        const scheduleId = CURRENT_USER?.activeScheduleId || EXAM_STATE.schedule?.id;
        const nis = CURRENT_USER?.nis;
        if (scheduleId && nis) {
          const sessionId = `${scheduleId}_${nis}`;
          const ansDoc = await getDoc(getPublicDoc("Jawaban Siswa", sessionId));
        ...
        console.error('cancelExamSession failed', err);
        showNotification("Gagal", err.message || "Gagal membatalkan sesi ujian.", "danger");
      } finally {
        toggleLoader(false);
      }
    }
    ...
    async function initStudentExamView() {
      ...
      try {
        ...
        if (s) {
          ...
          EXAM_STATE.scrambledQuestions.forEach(q => { EXAM_STATE.scrambledOptions[q.id] = [...(q.opsi || [])].sort((a, b) => a.key.localeCompare(b.key)); });
        ...
        if (sessionStorage.getItem('cbt-exam-active') === '1' && wasPageReloaded() && EXAM_STATE.scrambledQuestions.length > 0) {
          showNotification("Curang Terdeteksi", "Refresh halaman tidak diizinkan. Ujian otomatis dikirim.", "danger");
          autoSubmitExam("Refresh Halaman");
          return;
        }
    ...
    function renderExamQuestion() {
      ...
      if (tEl) {
        ...
        cleanSoal = cleanSoal.replace(/kunci\s*(?:jawaban)?\s*[:\-]?\s*[A-E]/gi, '');
      ...
      if (oc) {
        ...
        so.forEach(opt => {
          ...
          wrapper.innerHTML = `
            ...
            <label for="${optId}" class="group option-label flex items-start py-2.5 px-3.5 rounded-xl border-2 border-slate-300 dark:border-slate-600 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm cursor-pointer transition-all active:scale-95 active:translate-y-[1px] duration-100 hover:bg-white dark:hover:bg-slate-700 hover:border-[#234198]/50 dark:hover:border-blue-500/50 shadow-sm w-full">
              <span class="option-letter shrink-0 w-7 h-7 text-sm rounded-full border-2 border-slate-300 dark:border-slate-600 flex items-center justify-center font-bold text-slate-600 dark:text-slate-300 mr-3 transition-all group-hover:border-[#234198] group-hover:text-[#234198] dark:group-hover:border-blue-400 dark:group-hover:text-blue-400 bg-black/5 dark:bg-white/5">${opt.key}</span>
              <div class="option-text text-base font-semibold pt-0.5 leading-snug transition-all text-justify text-slate-800 dark:text-slate-200">${optTextHtml}</div>
            </label>`;
          const radio = wrapper.querySelector('input');
          radio.addEventListener('change', () => {
            EXAM_STATE.answers[q.id] = opt.key;
            saveExamStateToLocal();
            renderExamQuestion();
            syncStudentActiveProgress();
          });
    ...
    async function syncStudentActiveProgress() {
      ...
      try {
        ...
        const answeredCount = Object.keys(EXAM_STATE.answers).length;
        const totalCount = EXAM_STATE.scrambledQuestions.length;
    ...
    function updateExamProgressUI() {
      ...
      let greenCount = 0;
      let raguCount = 0;
      EXAM_STATE.scrambledQuestions.forEach(q => {
        if (EXAM_STATE.doubts[q.id]) raguCount++;
        else if (EXAM_STATE.answers[q.id] !== undefined) greenCount++;
      });
      const belumCount = t - greenCount - raguCount;
      const totalAnswered = Object.keys(EXAM_STATE.answers).length;
      ...
      if (modalText) {
        if (isReady) {
          ...
          modalText.innerHTML = `Anda masih memiliki <strong class="text-slate-700 dark:text-slate-200">${belumCount} soal</strong> yang belum dijawab dan <strong class="text-accent">${raguCount} soal</strong> ragu-ragu. Yakin ingin mengakhiri ujian sekarang?`;
        }
      }
    ...
    function renderDesktopMapGrid() {
      const renderGridItems = (containerId) => {
        ...
        EXAM_STATE.scrambledQuestions.forEach((q, idx) => {
          ...
          btn.type = 'button';
          btn.classList.add('question-nav-button');
          btn.onclick = () => {
          ...
          if (isFlagged) {
            ...
            classes += 'bg-transparent dark:bg-transparent text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700';
          }

          classes += isMobile ? ' p-1 text-[11px] sm:text-xs ' : ' text-sm ';
          classes += ' question-nav-button';
    ...
    function handleLogout() {
      mySessionStorage.removeItem('cbt-session');
      if (CURRENT_USER && CURRENT_USER.activeScheduleId && CURRENT_USER.nis) {
